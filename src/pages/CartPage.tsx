  ...
  if (items.length === 0) {
    return (
      <div className="bg-slate-50 min-h-[calc(100vh-4rem)] flex items-center justify-center p-4">
        <div className="container mx-auto max-w-2xl text-center bg-white p-12 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
          <h1 className="text-[1.75rem] font-bold font-display mb-4 text-[var(--navy-800)]">Your Cart is Empty</h1>
          <p className="text-slate-500 mb-8 text-lg">Looks like you haven't added any premium gifts to your cart yet.</p>
          <Link to="/shop" className={buttonVariants({ size: "lg", className: "bg-[var(--navy-800)] hover:bg-slate-800 text-white rounded-xl h-11 px-8 font-bold text-lg shadow-sm" })}>
            Explore Collections
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="bg-slate-50 min-h-[calc(100vh-4rem)] py-12">
      <div className="container mx-auto px-4 md:px-8 max-w-[80rem]">
        <h1 className="text-3xl md:text-[2rem] font-bold font-display tracking-tight mb-6 text-[var(--navy-800)]">Shopping Cart</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-6">
            {items.map(item => (
              <div key={item.productId} className="flex gap-6 p-5 sm:p-6 bg-white border border-slate-200 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                <div className="h-28 w-28 sm:h-36 sm:w-36 rounded-xl bg-slate-100 overflow-hidden flex-shrink-0 border border-slate-200">
                  <img 
                    src={item.image} 
                    alt={item.name} 
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    <div className="flex justify-between items-start gap-4">
                      <Link to={`/product/${item.productId}`} className="font-bold font-display text-xl text-[var(--navy-800)] hover:text-[var(--gold-500)] transition-colors line-clamp-2">{item.name}</Link>
                    ...
                    {item.customization?.enabled && (
                       <div className="mt-3 p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-3">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-bold bg-[var(--gold-500)]/10 border border-[var(--gold-500)]/20 text-[var(--navy-800)] px-2.5 py-1 rounded-md uppercase tracking-widest">Customized</span>
                          </div>
                        ...
                        {item.customization.customText && (
                           <div className="flex text-xs items-center">
                             ...
                             <span className="font-bold text-[var(--navy-800)]">"{item.customization.customText}"</span>
                           </div>
                        )}
                        <div className="flex text-xs items-center">
                          <span className="font-bold text-slate-500 w-16">Charge:</span>
                          <span className="font-bold text-[var(--gold-500)]">{formatCurrency(item.customization.charge)}</span>
                        </div>
                     </div>
                  )}
                ...
                <div className="flex items-center justify-between mt-4">
                  <div className="flex items-center gap-3 bg-slate-50 rounded-xl p-1 border border-slate-200 shadow-inner">
                    <button onClick={() => handleQuantity(item, -1)} className="p-1.5 hover:bg-white hover:text-[var(--navy-800)] rounded-lg text-slate-500 transition-colors shadow-sm"><Minus className="h-4 w-4" /></button>
                    <span className="w-8 text-center font-bold text-[var(--navy-800)] text-sm">{item.quantity}</span>
                    <button onClick={() => handleQuantity(item, 1)} className="p-1.5 hover:bg-white hover:text-[var(--navy-800)] rounded-lg text-slate-500 transition-colors shadow-sm"><Plus className="h-4 w-4" /></button>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-2xl text-[var(--navy-800)]">{formatCurrency(((item.basePrice + (item.customization?.charge || 0)) + calculateGST(item.basePrice + (item.customization?.charge || 0), item.gstPercent)) * item.quantity)}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.04)] h-fit sticky top-28">
          <h2 className="text-2xl font-bold font-display mb-6 text-[var(--navy-800)]">Order Summary</h2>
          <div className="space-y-4 mb-8 text-sm">
            <div className="flex justify-between items-center">
              <span className="text-slate-500 font-bold uppercase tracking-wider text-[11px]">Subtotal</span>
              <span className="font-bold text-[var(--navy-800)] text-base">{formatCurrency(getSubTotal())}</span>
            </div>
            <div className="flex justify-between items-center">
          ...
          <div className="border-t border-slate-200 pt-6 mb-8 flex justify-between items-end">
            ...
            <span className="font-bold font-display text-[1.75rem] text-[var(--navy-800)]">{formatCurrency(getGrandTotal())}</span>
          </div>

          <Link to="/checkout" className={buttonVariants({ size: "lg", className: "w-full text-base font-bold gap-2 bg-[var(--gold-500)] hover:bg-[var(--gold-400)] text-[var(--navy-800)] rounded-xl h-11 shadow-sm transition-all" })}>
